...
import {
  ...
  resolveRisk, RISK_EUR, ACCOUNT_CCY, controleerValuta,
} from './session.js';
...
async function handleOrder(o) {
  ...
  if (MAX_RISK_TOTAL > 0 && total + riskEur > MAX_RISK_TOTAL) {
    const r = `MAX_RISK_TOTAL bereikt (${total.toFixed(2)} + ${riskEur} > ${MAX_RISK_TOTAL})`;
    await db.pool.query('UPDATE signals SET status=$2, reason=$3 WHERE id=$1', [sig.id, 'rejected', r]);
    return { slot_id: o.slot_id, ok: false, reason: r };
  }
  ...
  try {
    ...
    console.log(`[Order] ${o.slot_id} ${o.action} ${mt5} ${sizing.lots} lots @ ${res.fill} ` +
                `SL ${res.sl} TP ${res.tp} | risk ${ACCOUNT_CCY} ${sizing.riskAmount}` +
...
app.get('/health', async (_req, res) => {
  try {
    ...
    res.json({
      ...
      limits: { max_open: MAX_OPEN || 'uit', max_risk_total: MAX_RISK_TOTAL || 'uit', risk_per_trade: RISK_EUR, ccy: ACCOUNT_CCY },
      risk_per_trade: `${ACCOUNT_CCY} ${RISK_EUR} vast`,
    });
...
app.get('/api/system', async (_req, res) => {
  const out = {
    ...
    limits: { max_open: MAX_OPEN || 'uit', max_risk_total: MAX_RISK_TOTAL || 'uit', risk_per_trade: RISK_EUR, ccy: ACCOUNT_CCY, max_basis_pct: MAX_BASIS_PCT },
    risk_per_trade: `${ACCOUNT_CCY} ${RISK_EUR} vast`,
...
(async () => {
  try {
    ...
    try {
      ...
      // Valutacontrole. Staat ACCOUNT_CCY verkeerd, dan is elke lotgrootte fout
      // en merk je dat anders pas als er geld weg is.
      const ccyWarn = controleerValuta(info?.currency);
      if (ccyWarn) await db.logError('boot.currency', ccyWarn);
      else console.log(`[MT5] rekeningvaluta ${ACCOUNT_CCY} komt overeen`);
